...
def show_config():
    ...
    print("="*60)optim as optim
from torch.utils.data import DataLoader, Subset
from sklearn.model_selection import train_t        print(f"\n   📊 Resultados Época {epoch+1}:")
    ...
    print("="*60) config import (IMAGES_DIR, MASKS_DIR, RANDOM_SEED, N_EPOCHS, BATCH_SIZE, 
                   MAMBA_VARIANT, IMAGE_SIZE, NORMALIZE_METHOD, USE_DATA_AUGMENTATION, NUM_WORKERS,
                   MAX_SAMPLES, USE_CACHE, LOG_INTERVAL, USE_LIGHTWEIGHT_MODEL)
from model.unet_mamba_variants import UNetMamba
from utils.metrics import dice_score
from preprocessing.dataset_v2 import HepaticVesselDatasetV2
import time
...
print("="*60)
print("� ENTRENAMIENTO UNet + Mamba - CONFIGURACIÓN OPTIMIZADA")
print("="*60)
print(f"🔁 Variante Mamba: {MAMBA_VARIANT}")
print(f"🖼️  Tamaño de imagen: {IMAGE_SIZE}x{IMAGE_SIZE}")
print(f"📦 Batch size: {BATCH_SIZE}")
...
def main():
    ...
    for epoch in range(N_EPOCHS):
        ...
        if val_dice > best_dice:
            best_dice = val_dice
            torch.save(model.state_dict(), f"best_hepatic_model_{MAMBA_VARIANT}.pth")
            print(f"   💾 Nuevo mejor modelo guardado! Dice: {best_dice:.4f}")

    # Guardar modelo final
    torch.save(model.state_dict(), f"final_hepatic_model_{MAMBA_VARIANT}.pth")
